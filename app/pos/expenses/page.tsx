  ...
  const handleSave = async () => {
    ...
    if (data.success) {
      ...
      if (editing) { setDialogOpen(false) }
        ...
        setForm({ ...form, description: "", amount: "" }); setKeepOpen(true)
      }
      ...
      <Dialog open={dialogOpen} onOpenChange={(v) => { if (!v) setDialogOpen(false) }}>
        <DialogContent className="sm:max-w-md">
          ...
          <div className="space-y-4 py-2">
            {savedMsg && <div className="bg-green-50 text-green-700 text-sm rounded-lg px-3 py-2 border border-green-200">{savedMsg}</div>}
            {!editing && (
                ...
                <div className="flex flex-wrap gap-2">
                  {TEMPLATES.map((t) => (
                    <button key={t.label} onClick={() => applyTemplate(t.desc)}
                      className={cn("px-3 py-1.5 rounded-lg text-xs font-medium border transition-all active:scale-95",
                        form.description === t.desc ? "bg-amber-100 border-amber-400 text-amber-800" : "bg-gray-50 border-gray-200 text-gray-600 hover:border-amber-300"
                      )}>{t.label}</button>
                  ))}
                </div>
              </div>
